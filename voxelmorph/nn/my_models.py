...
class BasicUNet(nn.Module):
    ...
    def __init__(
        ...
        normalizations: Union[
            Sequence[Union[Callable, str]], Callable, str, None
        ] = None,
        activations: Union[
            Sequence[Union[Callable, str]], Callable, str, None
        ] = nn.ReLU,
        ...
        # Asymmetric: [[down], [up]]
        if isinstance(nb_features[0], Sequence) and isinstance(
            nb_features[1], Sequence
        ):
        ...
        self.downsampling_conv_blocks, down_actual_channels = (
            ne.utils.downsampling_conv_blocks(
                ndim=ndim,
        ...
        skip_channels_list = (
            list(reversed(down_actual_channels)) if skip_connections else None
        )

        # Upsampling convolutional blocks (returns blocks and actual channel counts)
        self.upsampling_conv_blocks, up_actual_channels = (
            ne.utils.upsampling_conv_blocks(
                ndim=ndim,
    ...
    def forward(self, feature_tensor: torch.Tensor):
        ...
        if self.skip_connections:
            for block, skip in zip(
                self.upsampling_conv_blocks, reversed(skip_connections)
            ):
